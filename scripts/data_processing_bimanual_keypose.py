...
# 0.3
BUFFER_SIZE = 10
STOP_TH = 0.005 # Threshold to decide if a gripper stops
...
def keypoint_discovery(trajectories, buffer_size=5):
    ...
    Args:
        trajectories: a list of 1-D np arrays.  Each array is
            4-dimensional (x, y, z, raw, pitch, yaw, openness).
    ...
    # print("_local_max_A: ", _local_max_A)
    ...
    # print("local_max_A: ", local_max_A)
    pad_gripper_motion_changed = np.pad(gripper_motion_changed, (1, 1), mode='symmetric')
    gripper_motion_changed_ids = np.where(
        pad_gripper_motion_changed[:-1]
        != pad_gripper_motion_changed[1:]
    )[0]
    print("gripper_motion_changed_ids: ", gripper_motion_changed_ids)

    # waypoints are frames with changing gripper states
    gripper_state_changed_ids = gripper_state_changed(trajectories)

    print("gripper_state_changed_ids: ", gripper_state_changed_ids)

    last_frame = [len(trajectories) - 1]
    
    # distance , local maxa ??
    gripper_state_changed_ids = gripper_state_changed_ids.tolist()
    gripper_motion_changed_ids = gripper_motion_changed_ids.tolist() 

    potential_keyframe_inds = (
        # local_max_A +
        gripper_state_changed_ids +
        gripper_motion_changed_ids
        # + last_frame
    )
    potential_keyframe_inds = np.unique(potential_keyframe_inds)

    keyframe_inds = [ 0 ]
    for idx in potential_keyframe_inds:
    ...
    print("keyframe_inds: ", keyframe_inds)
...
def process_episode(data, cam_extrinsic, o3d_intrinsic, original_image_size, resized_intrinsic_o3d, resized_image_size, bound_box, left_bias, left_tip_bias, right_bias, right_tip_bias):
    ...
    # trajectories = np.concatenate( [left_trajectories, left_trajectories], axis = 1)
    ...
    # frame_ids = np.unique(np.concatenate([left_keyframe_inds, left_keyframe_inds]))

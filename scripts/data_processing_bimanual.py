...
def main():
    ...
    for task_idx, task_name in enumerate(tasks, 0):
        ...
        for data_idx, file in enumerate(res):
            ...
            gripper_max = max(left_gripper_max, right_gripper_max)
            gripper_min = min(left_gripper_min, right_gripper_min)
            ...
            for point in data:
                ...
                left_openess = ( float(point["left_pos"][6]) - gripper_min ) / (gripper_max - gripper_min )
                ...
                right_openess = ( float(point["right_pos"][6]) - gripper_min ) / (gripper_max - gripper_min )
            ...
            left_trajectory_np = np.array(left_trajectory)
            right_trajectory_np = np.array(right_trajectory)
            max_x = np.max( np.stack( [ left_trajectory_np[:,0], right_trajectory_np[:,0] ]) )
            max_y = np.max( np.stack( [ left_trajectory_np[:,1], right_trajectory_np[:,1] ]) )
            max_z = np.max( np.stack( [ left_trajectory_np[:,2], right_trajectory_np[:,2] ]) )

            min_x = np.min( np.stack( [left_trajectory_np[:,0], right_trajectory_np[:,0] ]) )
            min_y = np.min( np.stack( [left_trajectory_np[:,1], right_trajectory_np[:,1] ]) )
            min_z = np.min( np.stack( [left_trajectory_np[:,2], right_trajectory_np[:,2] ]) )
            ...
            print("trajectories: ", trajectories.shape)
            print("transform: ", len(right_trajectory)-1)

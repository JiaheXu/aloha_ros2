...
def main():
    ...
    data_idx =  39
    episode = np.load("./ep{}.npy".format(data_idx) , allow_pickle = True)
    print("loading: ", "./ep{}.npy".format(data_idx))
    ...
    sample = np.load("./step_0.npy" , allow_pickle = True)
    sample = sample.item()
    rgb = sample["rgb"]
    rgb = rgb[0,0].numpy()
    ...
    real_world_pcd = o3d.geometry.PointCloud()
    real_world_pcd.colors = o3d.utility.Vector3dVector( pcd_rgb )
    real_world_pcd.points = o3d.utility.Vector3dVector( pcd_xyz )

    visualize_pcds( [pcd] )

    visualize_pcds( [real_world_pcd] )

    visualize_pcds( [pcd, real_world_pcd] )
